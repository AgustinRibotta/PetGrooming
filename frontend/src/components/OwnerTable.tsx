    ...
    <table className="min-w-full border border-gray-300 rounded overflow-hidden">
      <thead>
        <tr className="bg-indigo-100 text-indigo-800">
          ...
          <th className="border px-4 py-2 text-left">Pet Count</th>
        ...
        {owners.length > 0 ? (
          owners.map((owner) => (
            <tr key={owner.id} className="hover:bg-indigo-50 transition">
              ...
              <td className="border px-4 py-2">{owner.phoneNumber}</td>
              <td className="border px-4 py-2">{owner.petCount ?? 0}</td>
